...
class HybridSPEALAHC(FJSSPAlgorithm):
    ...
    @staticmethod
    def _get_neighbor_static(encoding: WorkerEncoding, machines_for_ops, candidate: Candidate):
        """
        Static version of the neighbor generator. 
        Does not use 'self' so it can be pickled for multiprocessing.
        """
        # Deepcopy only the list of operations to save time/memory
        new_ordered_ops = copy.deepcopy(candidate.ordered_ops)
        idx1 = random.randrange(len(new_ordered_ops))
        mutation_type = random.random()

        # 1. Change machine and worker assignment
        if mutation_type < 0.4:
            op = new_ordered_ops[idx1]
            usable_machines = machines_for_ops[op.operation_index]
            new_m = random.choice(usable_machines)
        ...
        # 2. Swap the job order of two operations
        elif mutation_type < 0.8:
            idx2 = random.randrange(len(new_ordered_ops))
            tmp_job = new_ordered_ops[idx1].job_index
            new_ordered_ops[idx1].job_index = new_ordered_ops[idx2].job_index
        ...
        # 3. Shift operation to a new position
        else:
            target_idx = random.randint(0, len(new_ordered_ops) - 1)
            job_ids = [op.job_index for op in new_ordered_ops]
            moving_job = job_ids.pop(idx1)
    ...
    @staticmethod
    def _local_search_lahc_static(instance: Instance, encoding: WorkerEncoding, machines_for_ops, lahc_iters, lahc_l):
        """
        The static local search function that runs on each worker process.
        """
        # Convert to Candidate once at the start
        current_cand = instance.to_candidate()
        history = [current_cand.makespan] * lahc_l
        curr_m = current_cand.makespan
        for i in range(lahc_iters):
            # Use the static helper
            neighbor = HybridSPEALAHC._get_neighbor_static(encoding, machines_for_ops, current_cand)
            v = i % lahc_l
            ...
            history[v] = curr_m
        # Sync results back to the Instance object
        job_seq, m_seq, w_seq = current_cand.get_sequences()
        instance.operation_sequence = job_seq
        instance.worker_machine_sequence = list(zip(m_seq, w_seq))
        instance.makespan = current_cand.makespan
    ...
    def solve(self, encoding: WorkerEncoding) -> tuple[Candidate, list]:
        ...
        local_search_func = partial(
            self._local_search_lahc_static, 
            encoding=encoding, 
            machines_for_ops=machines_for_ops,
            lahc_iters=self.lahc_iters,
            lahc_l=self.lahc_l
        )
        ...
        for gen in range(1, self.max_generations + 1):
            ...
            with Pool(processes=os.cpu_count()) as pool:
                population = pool.map(local_search_func, population)
